% SWAPMODEL2D returns a structure for a three-component model
...
function model = SwapModel2D(boundary, maxIter)
    model.name = 'Swap model 2D';
...
function [p, pSep] = SwapModel2DPDF(data, g, B, sd)
  ...
  pA = (1-g-B).*mvnpdf(data.errors',[0,0],[sd^2,sd^2]); % prob of target
  ...
  for i=1:max(numDistractorsPerTrial)
    pdfOut = mvnpdf(data.errors', data.distractors((i*2-1):i*2,:)', diag([sd^2,sd^2],0)); % prob of misbind to this distractor
  ...
  %distractor errors
    for d=1:(size(displayInfo.distractors,1)/2)
        if any(whichType==d)
            ...
            distResps = mvnrnd(distLoc', [params{3}^2,params{3}^2], n);
