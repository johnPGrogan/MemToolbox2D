% SWAPMODELCOVAR2D returns a structure for a four-component model
% with guesses and swaps, with VAR and COVAR. 
...
function model = SwapModelCovar2D(boundary, maxIter)
    model.name = 'Swap model Covar 2D';
	model.paramNames = {'g', 'B', 'sd', 'coSd'};
	model.lowerbound = [0 0 0 -Inf]; % Lower bounds for the parameters
	model.upperbound = [1 1 Inf Inf]; % Upper bounds for the parameters
	model.movestd = [0.02, 0.02, 0.5, 0.1];
    model.pdf = @SwapModel2DPDF;
    model.modelPlot = @model_plot;
    model.generator = @SwapModel2DGenerator;
    model.start = [0.2, 0.1, 20, 0;  % g, B, sd
                   0.4, 0.1, 40, 1;  % g, B, sd
                   0.1, 0.5, 60, 5]; % g, B, sd
...
function [p, pSep] = SwapModel2DPDF(data, g, B, sd, coSd)
  [~,nonDef] = cholcov([sd coSd; coSd, sd],0);  % check covar matrix is positive definite
  ...
  pA = (1-g-B).*mvnpdf(data.errors',[0,0],[sd^2, coSd^2; coSd^2, sd^2]); % prob of target
  ...
  for i=1:max(numDistractorsPerTrial)
    pdfOut = mvnpdf(data.errors', data.distractors((i*2-1):i*2,:)', [sd^2,coSd^2; coSd^2, sd^2]); % prob of misbind to this distractor
...
if params{1} + params{2} > 1
  ...
  targResps = mvnrnd(targs', [params{3}^2,params{4}^2;params{4}^2,params{3}^2], n); % generate target response locations
  ...
  if any(toReplace,'all') %either resample (default) or apply boundary, depending on displayInfo fields
      targResps(toReplace,:) = ResampleOrBound(targResps(toReplace,:), targs(:,toReplace), [params{3}^2,params{4}^2;params{4}^2,params{3}^2], model, dimensions);
  end
  ...
  %distractor errors
    for d=1:(size(displayInfo.distractors,1)/2)
        if any(whichType==d)
            ...
            distResps = mvnrnd(distLoc', [params{3}^2,params{4}^2;params{4}^2,params{3}^2], n);

            toReplace = any(distResps < 0 | distResps > dimensions',2); %check if out of bounds
            if any(toReplace,'all') %either resample (default) or apply boundary, depending on displayInfo fields
                distResps(toReplace,:) = ResampleOrBound(distResps(toReplace,:), distLoc(:,toReplace), [params{3}^2,params{4}^2;params{4}^2,params{3}^2], model, dimensions);
            end
