% SWAPMODELSepPrec2D returns a structure for a model with separate
% precisions for targets and misbinds, and standard guesses and swaps.
...
function model = SwapModelSepPrec2D(boundary, maxIter)
    model.name = 'Swap model 2D with 2 precisions';
	model.paramNames = {'g', 'B', 'sdT','sdB'};
...
function [p, pSep] = SwapModel2DPDF(data, g, B, sdT, sdB)
  ...
  pA = (1-g-B).*mvnpdf(data.errors',[0,0],[sdT^2,sdT^2]); % prob of target
  ...
  for i=1:max(numDistractorsPerTrial)
    pdfOut = mvnpdf(data.errors', data.distractors((i*2-1):i*2,:)', diag([sdB^2,sdB^2],0)); % prob of misbind to this distractor
...
if params{1} + params{2} > 1
  ...
  targResps = mvnrnd(targs', [params{3}^2,params{3}^2], n); % generate target response locations
  ...
  if any(toReplace,'all') %either resample (default) or apply boundary, depending on displayInfo fields
      targResps(toReplace,:) = ResampleOrBound(targResps(toReplace,:), targs(:,toReplace), [params{3}^2,params{3}^2], model, dimensions);
  end
  ...
  %distractor errors
    for d=1:(size(displayInfo.distractors,1)/2)
        if any(whichType==d)
            ...
            distResps = mvnrnd(distLoc', [params{4}^2,params{4}^2], n);

            toReplace = any(distResps < 0 | distResps > dimensions',2); %check if out of bounds
            if any(toReplace,'all') %either resample (default) or apply boundary, depending on displayInfo fields
                distResps(toReplace,:) = ResampleOrBound(distResps(toReplace,:), distLoc(:,toReplace), [params{4}^2,params{4}^2], model, dimensions);
            end
