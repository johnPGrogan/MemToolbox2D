% SWAPMODELXYCOVAR2D returns a structure for a five-component model
% with guesses and swaps, with X & Y VAR and 1 COVAR. 
...
function model = SwapModelXYCovar2D(boundary, maxIter)
    model.name = 'Swap model X & Y Covar 2D';
	model.paramNames = {'g', 'B', 'sdX', 'sdY', 'coSd'};
	model.lowerbound = [0 0 0 0 0]; % Lower bounds for the parameters
	model.upperbound = [1 1 Inf Inf Inf]; % Upper bounds for the parameters
	model.movestd = [0.02, 0.02, 0.5, 0.5, 0.1];
    model.pdf = @SwapModel2DPDF;
    model.modelPlot = @model_plot;
    model.generator = @SwapModel2DGenerator;
    model.start = [0.2, 0.1, 20, 10, 0;  % g, B, sd
                   0.4, 0.1, 40, 50, 1;  % g, B, sd
                   0.1, 0.5, 60, 60, 5]; % g, B, sd
...
function [p, pSep] = SwapModel2DPDF(data, g, B, sdX, sdY, coSd)
  [~,nonDef] = cholcov([sdX^2 coSd^2; coSd^2, sdY^2],0);  % check covar matrix is positive definite
  if nonDef ~= 0 % if matrix is invalid
  ...
  pA = (1-g-B).*mvnpdf(data.errors',[0,0],[sdX^2, coSd^2; coSd^2, sdY^2]); % prob of target
  ...
  for i=1:max(numDistractorsPerTrial)
    pdfOut = mvnpdf(data.errors', data.distractors((i*2-1):i*2,:)', [sdX^2,coSd^2; coSd^2, sdY^2]); % prob of misbind to this distractor
...
if params{1} + params{2} > 1
  ...
  targResps = mvnrnd(targs', [params{3}^2,params{5}^2;params{5}^2,params{4}^2], n); % generate target response locations
  ...
  if any(toReplace,'all') %either resample (default) or apply boundary, depending on displayInfo fields
      targResps(toReplace,:) = ResampleOrBound(targResps(toReplace,:), targs(:,toReplace), [params{3}^2,params{5}^2;params{5}^2,params{4}^2], model, dimensions);
  end
  ...
  %distractor errors
    for d=1:(size(displayInfo.distractors,1)/2)
        if any(whichType==d)
            ...
            distResps = mvnrnd(distLoc', [params{3}^2,params{5}^2;params{5}^2,params{4}^2], n);

            toReplace = any(distResps < 0 | distResps > dimensions',2); %check if out of bounds
            if any(toReplace,'all') %either resample (default) or apply boundary, depending on displayInfo fields
                distResps(toReplace,:) = ResampleOrBound(distResps(toReplace,:), distLoc(:,toReplace), [params{3}^2,params{5}^2;params{5}^2,params{4}^2], model, dimensions);
            end
