% TESTALLMODELS2D runs tests to be sure we are correctly sampling from and recovering the data
% for all the default 2D models.
%
%   TestAllModels2D(numTrials, numItemsPerTrial);
% 
...
function TestAllModels2D(numTrials, numItemsPerTrial)
  ...
  models = {
    ...
    WithBias2D(StandardMixtureModel2D);
    WithRadialBias2D(StandardMixtureModel2D);
    SwapModel2D();
    SwapModelSepPrec2D();
    SwapModelCovar2D();
    SwapModelXYVar2D();
    SwapModelXYCovar2D();
    VariablePrecisionModel2D();
    EnsembleIntegrationModel2D();
    ContinuousResourceModel2D();
};
