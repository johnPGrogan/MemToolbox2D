% TESTSAMPLINGANDFITTING2D sample from a model and then attempt to recover parameters
%
%  [paramsOut, lowerCI, upperCI] = ...
%      TestSamplingAndFitting2D(model, paramsIn, numTrials, ...
...
%   [paramsOut, lowerCI, upperCI] = TestSamplingAndFitting2D(model, paramsIn, numTrials, numItems)
%
% or just:
%   model = WithBias2D(StandardMixtureModel2D);
%   [paramsOut, lowerCI, upperCI] = TestSamplingAndFitting2D(model)
%
...
function [paramsOut, lowerCI, upperCI] = ...
    TestSamplingAndFitting2D(model, paramsIn, numTrials, numItems, varargin)
  ...
  for i = 1:length(numTrials)
    ...
    if length(numItems)>1
      ...
      displays = GenerateDisplays2D(numTrials(i), numItemsVec, 2);
    else
      displays = GenerateDisplays2D(numTrials(i), numItems, 2);
    end

    % Generate error data for these displays:
    data = displays;
    if isfield(model, 'isTwoAFC')
      data.afcCorrect = SampleFromModel2D(model, paramsIn, [1,numTrials(i)], displays);
    else
      data.errors = SampleFromModel2D(model, paramsIn, [1,numTrials(i)], displays);
    end
