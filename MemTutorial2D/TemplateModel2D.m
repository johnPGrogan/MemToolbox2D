% TEMPLATEMODEL2D is a 2D model template whose comments explain each of the required
% and optional components. (It happens to be NoGuessingModel2D, with bias)
%
...
function model = TemplateModel2D()

  % The model should be given a unique name. This name is used by MemFit2D and
  % the model testing functions to let the user know which model is being
  % tested. Example model names are 'Slots+averaging model 2D' and 'Swap model 2D'.
  model.name = 'Template model 2D';
...
function [p, pSep] = modelPDF(data, p1, p2)
    dimensions = GetModelDims(data); % get the model dimensions

	
    pA = mvnpdf(data.errors',[p1, p1], [p2.^2, p.^2]); % get the probability of 
	
	% the pSep output is useful to return to allow you to classify responses as targ/guess etc
	pSep = [pA]; % cat each type of response into columns. 
	p = nansum(p,2); % sum across diff resp types for each trial (row)
end

% The model's random number generator function
function r = modelGenerator(parameters, dims, displayInfo)
	r = mvnpdf([parameters{1} parameters{1}], [parameters{2}^2 parameters{2}^2], dims);
end
